...
impl MdbxEnvironment {
    ...
    pub fn new(path: &str, size: usize, max_dbs: u32) -> Result<Environment, LmdbError> {
        Ok(Environment::Persistent(
            MdbxEnvironment::new_lmdb_environment(path, size, max_dbs, None)?,
        ))
    }
    ...
    pub fn new_with_max_readers(
        ...
        Ok(Environment::Persistent(
            MdbxEnvironment::new_lmdb_environment(path, size, max_dbs, Some(max_readers))?,
        ))
    }
    ...
    pub(super) fn new_lmdb_environment(
        ...
        let mut geo = libmdbx::Geometry::default();
        geo.size = Some(0..size);

        env.set_geometry(geo);

        let mut db_flags = libmdbx::EnvironmentFlags::default();
        db_flags.no_rdahead = true;
        db_flags.mode = libmdbx::Mode::ReadWrite {
...
impl<'env> MdbxWriteTransaction<'env> {
    ...
    pub(super) fn write_cursor<'txn, 'db>(&'txn self, db: &'db Database) -> MdbxWriteCursor<'txn> {
        ...
        MdbxWriteCursor {
            raw: RawWriteLmdbCursor { cursor: cursor },
        }
    }
}
...
impl<'txn, 'db> RawReadCursor for RawMDBXCursor<'txn> {
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.first().unwrap();
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.first().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.last().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.last().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.get_current().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.get_current().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.next().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.next().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.next_dup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.next_dup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.next_nodup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.next_nodup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.prev().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.prev().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.prev_dup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.prev_dup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rocursor.prev_nodup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> = rwcursor.prev_nodup().unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let key = AsDatabaseBytes::as_database_bytes(key);
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> =
                    rocursor.set_key(key.as_ref()).unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let key = AsDatabaseBytes::as_database_bytes(key);
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> =
                    rwcursor.set_key(key.as_ref()).unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
        ...
        V: FromDatabaseValue,
    {
        match self {
            Self::RoCursor(rocursor) => {
                let key = AsDatabaseBytes::as_database_bytes(key);
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> =
                    rocursor.set_range(key.as_ref()).unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
            ...
            Self::RwCursor(rwcursor) => {
                let key = AsDatabaseBytes::as_database_bytes(key);
                let result: Option<(Cow<[u8]>, Cow<[u8]>)> =
                    rwcursor.set_range(key.as_ref()).unwrap();
                let (key, value) = result?;
                Some((
                    FromDatabaseValue::copy_from_database(&key).unwrap(),
                    FromDatabaseValue::copy_from_database(&value).unwrap(),
...
impl<'txn, 'db> RawReadCursor for RawWriteLmdbCursor<'txn> {
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.first().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.last().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.get_current().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.next().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.next_dup().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.next_nodup().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.prev().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.prev_dup().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    where
        ...
        V: FromDatabaseValue,
    {
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.prev_nodup().unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    {
        let key = AsDatabaseBytes::as_database_bytes(key);
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.set_key(key.as_ref()).unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
    ...
    {
        let key = AsDatabaseBytes::as_database_bytes(key);
        let result: Option<(Cow<[u8]>, Cow<[u8]>)> = self.cursor.set_range(key.as_ref()).unwrap();
        let (key, value) = result?;
        Some((
            FromDatabaseValue::copy_from_database(&key).unwrap(),
            FromDatabaseValue::copy_from_database(&value).unwrap(),
...
/*
impl<'txn, 'db> WriteCursorTrait for MdbxCursor<'txn> {
    fn remove(&mut self) {
        match self.raw {
            RawMDBXCursor::RoCursor(rocursor) => {
                panic!("Called remove in a read cursor")
            }
            RawMDBXCursor::RwCursor(rwcursor) => rwcursor.del(mdbx::WriteFlags::APPEND).unwrap(),
        }
    }
}
*/
