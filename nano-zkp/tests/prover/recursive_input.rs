...
use nimiq_nano_zkp::utils::{pack_inputs, unpack_inputs};

const NUMBER_OF_BITS: usize = 704;
...
impl ConstraintSynthesizer<MNT4Fr> for InnerCircuit {
    ...
    fn generate_constraints(self, cs: ConstraintSystemRef<MNT4Fr>) -> Result<(), SynthesisError> {
        ...
        let red_pub_var = FqVarMNT6::new_input(cs.clone(), || Ok(&self.red_pub))?;

        let blue_pub_var = FqVarMNT6::new_input(cs, || Ok(&self.blue_pub))?;
...
impl ConstraintSynthesizer<MNT6Fr> for OuterCircuit {
    ...
    fn generate_constraints(self, cs: ConstraintSystemRef<MNT6Fr>) -> Result<(), SynthesisError> {
        ...
        // Allocate all the inputs.
        let red_pub_var = FqVarMNT4::new_input(cs.clone(), || Ok(&self.red_pub))?;

        let blue_pub_var = FqVarMNT4::new_input(cs, || Ok(&self.blue_pub))?;
        ...
        // Prepare inputs
        let proof_inputs = vec![red_var_bits, blue_var_bits];
...
fn recursive_input_works() {
    ...
    let red_pub: FqMNT6 = pack_inputs(red_priv.clone());
    ...
    let blue_pub: FqMNT6 = pack_inputs(blue_priv.clone());
    ...
    // Verify the proof.
    let inputs = vec![red_pub, blue_pub];

    assert!(Groth16::<MNT4_753>::verify(&vk, &inputs, &proof).unwrap());

    // Create inputs for the outer circuit.
    let red_pub: FqMNT4 = pack_inputs(red_priv);
    let blue_pub: FqMNT4 = pack_inputs(blue_priv);
